...
`YOURPROJECT.egg-info` directory that setuptools generates. It looks for that
directory in the root folder and in the direct subdirectory (like `src/` or
`plone/`).
...
For our test, we call the `main()` method, just like the
`dependencychecker` script would:
